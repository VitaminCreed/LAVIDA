...
class VosDataset(BaseVideoDataset):
    def __init__(
        self,
        data_root: Path | str='',
        list_root: Path | str='./lists/Vos',
        ...
        **kwargs,
        ...
        self.indexer_cls = VosClipsIndexer
        self.anomaly_ratio = anomaly_ratio
        self.n_anomalies = n_anomalies
        self.samples = make_vos_dataset(self.data_root, self.inference)
...
class VosClipsIndexer(ClipsIndexer):
    """Clips indexer for Vos dataset.
    ...
    def get_mask(self, 
        ...
        for pt in frames.int():
            ...
            if f_id in frames_object_class.keys(): 
                objs.append(frames_object_class[f_id])
        ...
        if objs:
            objs = random.choices(objs, k=random.randint(1, 2))
        
        unique_categories = sorted({category for item in objs for category in item})
        unique_obj_ids = sorted({x for obj in objs for x in chain(*obj.values())})
        ...
        anomaly_labels, anomaly_types = select_anomalies(
            anomaly_classes=unique_categories,
...
def make_vos_dataset(
    root: Path, 
    inference: bool = False
    ) -> DataFrame:
    """Constructs a DataFrame dataset for VOS.
    ...
    def reorganize_by_frames(objects):
        ...
        for obj_id, obj_data in objects.items():
            ...
            for frame in obj_data["frames"]:
                frames_dict[frame][category].append(int(obj_id))
        return frames_dict
