...
class MultiStepMultiMasksAndIous(nn.Module):
    def __init__(
        ...
        focal_gamma_obj_score=0.0,  # 0.8
        focal_alpha_obj_score=-1,  # -1
    ):
        """
    ...
    #     """
    #     targets_batch: [batch_size, num_objects, H, W]
    #     """
    ...
    def forward(self, pred_masks, pred_ious, object_score_logits, targets, target_obj):
        """
        pred_masks: [N, M, H, W]
        pred_ious: [N, M]
        object_score_logits: [N]
        targets: [N, H, W]
        target_obj: [N]
        """
        target_masks = targets.unsqueeze(1).float()
        assert target_masks.dim() == 4  # [N, 1, H, W]
        src_masks_list = [pred_masks]
        ious_list = [pred_ious]
        object_score_logits_list = [object_score_logits]
        ...
        losses["total_loss"] = self.reduce_loss(losses)
        return losses
